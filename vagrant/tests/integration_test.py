...
def get_ssh_config(vm_name):
    ssh_config_raw = subprocess.check_output(
        ["vagrant", "ssh-config", vm_name]).decode()
    config = {}
...
def test_ping_to_gateway_prod():
    output = ssh_command("client_prod", "ping -c 3 10.100.1.254")
    loss = packet_loss_from_ping(output)
    assert loss < 100, f"Ping failed: {loss}% packet loss\n{output}"


def test_ping_to_gateway_test():
    output = ssh_command("client_test", "ping -c 3 10.100.2.254")
    loss = packet_loss_from_ping(output)
    assert loss < 100, f"Ping failed: {loss}% packet loss\n{output}"


def test_ping_to_gateway_entw():
    output = ssh_command("client_entw", "ping6 -c 3 fd11:2:3:4::10")
    loss = packet_loss_from_ping(output)
    assert loss < 100, f"Ping failed: {loss}% packet loss\n{output}"
...
def test_nat64_ping_client_entw(client_prod_ip):
    output = ssh_command(
        "client_entw", f"ping -6 -c 3 64:ff9b::{client_prod_ip}")
    loss = packet_loss_from_ping(output)
    assert loss < 100, f"Ping failed: {loss}% packet loss\n{output}"
...
def test_dns_resolution_client_prod():
    output = ssh_command(
        "client_prod", "dig +short server1.azubi.dataport.de @localhost")
    assert "10.100.1.10" in output


def test_dns_resolution_client_test():
    output = ssh_command(
        "client_test", "dig +short server2.test.azubi.dataport.de @localhost")
    assert "10.100.2.10" in output


def test_dns_resolution_client_test_prod(client_prod_ip):
    output = ssh_command(
        "client_test", f"dig +short server1.azubi.dataport.de @{client_prod_ip}")
    assert "10.100.1.10" in output


def test_dns_forwarding_test_prod():
    output = ssh_command(
        "client_test", "dig +short server1.azubi.dataport.de @localhost")
    assert "10.100.1.10" in output


def test_dns_forwarding_prod_test():
    output = ssh_command(
        "client_prod", "dig +short debian.test.azubi.dataport.de @localhost")
...
def test_dns64_entw_test():
    output = ssh_command(
        "client_entw", "dig +short debian.test.azubi.dataport.de aaaa")
